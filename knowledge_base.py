...
if __name__ == "__main__":
    while True:
        user_status = input("Enter the status of knowledge bases to delete (FAILED, ACTIVE, DELETE_UNSUCCESSFUL) or 'QUIT' to exit: ").strip().upper()
        if user_status == 'QUIT':
            print("Exiting the program.")
            break
        elif user_status in ['FAILED', 'ACTIVE', 'DELETE_UNSUCCESSFUL']:
            delete_knowledge_bases_with_status(user_status)
        else:
            print("Invalid status. Please enter a valid status (FAILED, ACTIVE, DELETE_UNSUCCESSFUL) or 'QUIT' to exit.")
